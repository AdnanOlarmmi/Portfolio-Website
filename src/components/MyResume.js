...
const MyResume = () => {
  ...
  return (
    <section id="resume" className="myresume flex-column">
      <h6 className="myprofile__subheading">
        {experienceYears}
        + YEARS OF EXPERIENCE
      </h6>
      <h1 className="myprofile__heading">My Resume</h1>
      <div className="myresume__item">
        {resumes.map((resume, index) => <span className={`myresume__heading ${activeSlide.props.dataset === resume.name ? 'active' : ''}`} onClick={() => { handleClick(index); }}>{resume.name === 'Skills' ? 'Professional Skills' : resume.name}</span>)}
      </div>
      <div>{activeSlide}</div>
    </section>
  );
